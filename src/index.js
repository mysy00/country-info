...
const manageData = data => {
	...
	data.forEach((country, index) => {
		...
		const formattedBorders =
			JSON.stringify(borders).replace(/"|\[|]/g, "") || "None";
		const foundData = `
      <div class="country" data-result="${index}">
        <h3 class="country-title" tabindex="${index +
					3}">${name} <img class="country-flag" src="${flag}" alt></h3>
...
countrySearch.addEventListener("submit", e => {
	const searchBy = document.querySelector(".js-settings-modifier:checked")
		.value;
	e.preventDefault();
	toggleLoader(true);
	if (searchBy === "name") {
